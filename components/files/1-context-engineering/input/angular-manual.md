...
to the code in those directories. For modern Angular applications with **standalone components**, follow this recommended structure:
...
│  │  └─ core.providers.ts     # Core providers for app.config.ts
│  ├─ shared/                  # Shared standalone components, directives, pipes
│  │  ├─ components/
│  │  │  ├─ breadcrumb/
...
│  │  │  ├─ header/
│  │  │  ├─ sidebar/
│  │  │  └─ file-cover/        # Example placeholder component
│  │  │     ├─ file-cover.component.ts
│  │  │     ├─ file-cover.component.html
│  │  │     └─ file-cover.component.css
...
│  │  │     └─ template.model.ts
│  │  ├─ edit-template/
│  │  │  ├─ edit-template.component.ts
│  │  │  ├─ edit-template.component.html
...
│  │  │     └─ variable-selector/
│  │  └─ publish-template/
│  │     ├─ publish-template.component.ts
│  │     ├─ publish-template.component.html
│  │     ├─ publish-template.component.css
│  │     └─ components/
│  │        └─ publish-settings/
...
- **Core**: Contains singleton services and application-wide functionality
- **Shared**: Contains reusable components, services, and utilities used across features
- **Features**: Contains feature-specific components organized by domain
- **Standalone components**: Prefer standalone components over NgModules for better tree-shaking and simpler imports

For example, a template management system might look like this:
...
example, avoid creating directories like `components`, `directives`, and `services` at the root level.
...
**Example of modern dependency injection pattern:**
...
@Component({
  ...
  template: `
    ...
    <div class="actions">
      <button (click)="navigateToEdit()" [disabled]="!canEdit()">
        Edit Template
      </button>
    </div>
  `,
...
export class TemplateManagementComponent {
  ...
  // State signals
  title = signal('Template Management');
  loading = signal(false);
  error = signal<string | null>(null);
  templates = signal<Template[]>([]);
  
  // Computed properties
  canEdit = computed(() => this.templates().length > 0 && !this.loading());
  ...
  // Always include event handlers for template compatibility
  onInputEvent(event: Event): void {
    const target = event.target as HTMLInputElement;
    // Handle input properly with type assertion
  }
  
  onSelectChange(event: Event): void {
    const target = event.target as HTMLSelectElement;
    // Handle select change properly
  }
}
```

**Modern service pattern with error handling:**
...
export class TemplateService {
  ...
  // Always provide complete CRUD operations
  getTemplate(id: string): Observable<Template> {
    return this.http.get<Template>(`/api/templates/${id}`)
      .pipe(
        catchError(error => {
          this.notificationService.showError('Template not found');
  ...
  updateTemplate(id: string, template: Partial<Template>): Observable<Template> {
    return this.http.put<Template>(`/api/templates/${id}`, template);
  }
...
1. **Use standalone components** (as shown above) instead of NgModules for better tree-shaking
2. **Import only what you need** in the imports array
3. **Use meaningful file names** that match the component class name
4. **Group related files** in the same directory

**Example of a feature component with routing:**
...
Modern Angular applications often need to integrate with external libraries for enhanced functionality. Here's how to properly integrate third-party libraries while maintaining Angular best practices.

### Dynamic Imports for Better Performance

When using large libraries like rich text editors, use dynamic imports to improve initial bundle size:
